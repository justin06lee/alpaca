...
# The desktop app: the same binary in a bundle. Finder launches it with no
# terminal attached, which is what routes the bare binary into gui mode.
APP     := Alpaca
APPDIR  := build/$(APP).app
UNAME_S := $(shell uname -s)

.PHONY: all build install update uninstall where test test-live lint fmt cross clean run \
	app app-install icns

# The golden path: a bare `make` leaves `alpaca` runnable by name from any
# directory — and, on macOS, $(APP).app in /Applications for the Dock. There
# is no daemon to launch on this machine — serving happens wherever
# `alpaca serve` runs — so building and installing is the whole journey.
ifeq ($(UNAME_S),Darwin)
